<x-xml-layout>
    <div class="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        @if(!$msgs)
            <h3>Number of payments : {{count($payments->CdtTrfTxInf)}} | Declared : {{$file_info->NbOfTxs}}</h3>
            <h3>Total of payments : {{$payments->sum}} | Declared : {{$file_info->CtrlSum}}</h3>
        @endif
            ...
            @foreach($payments->CdtTrfTxInf as $payment)
                    <div class="space-y-2 xl:grid xl:grid-cols-4 xl:items-baseline xl:space-y-0">
                        <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4" role="alert">
                            <p class="font-bold">Payment Informations</p>
                            <p>{{$payment->Cdtr->Nm}}</p>
                            @foreach ($payment->PstlAdr as $line)
                                <p>{{$line}}</p>
                            @endforeach
